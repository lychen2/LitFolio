...
function HitBody({ hit, error }: { hit: SearchHit; error: unknown }) {
  const t = useT();
  return (
    <div className="min-w-0 flex-1">
      <div className="font-medium text-litera-text leading-snug">{hit.draft.title}</div>
...
function HitAction({
  ...
  if (!canAdd) {
    return (
      <span className="text-xs text-litera-mute italic whitespace-nowrap" title={t("topic.search.manualOnlyTitle")}>
