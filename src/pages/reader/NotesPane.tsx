    ...
    <div className="h-full flex flex-col bg-litera-paper/30">
      <div className="px-3 py-2 border-b border-litera-line flex items-center justify-between gap-2">
        <div className="text-xs uppercase tracking-wider text-litera-mute flex items-center gap-1.5">
          <NotebookPen className="h-3.5 w-3.5" /> 笔记
        </div>
      ...
      <textarea
        ...
        placeholder={initial.isLoading ? "加载笔记…" : "在此用 Markdown 写笔记…自动保存到 papers/<id>/note.md"}
...
function SaveStatus({ status, error }: { status: "saving" | "dirty" | "saved"; error: Error | null }) {
  if (error) {
    return <span className="text-[11px] text-red-400/90">✕ 保存失败:{error.message}</span>;
  }
  if (status === "saving") {
    return <span className="text-[11px] text-litera-accent2 flex items-center gap-1"><Loader2 className="h-3 w-3 animate-spin" /> 保存中…</span>;
  }
  if (status === "dirty") {
    return <span className="text-[11px] text-amber-400/80">● 未保存</span>;
  }
  return <span className="text-[11px] text-litera-mute flex items-center gap-1"><Check className="h-3 w-3 text-emerald-400" /> 已保存</span>;
}
