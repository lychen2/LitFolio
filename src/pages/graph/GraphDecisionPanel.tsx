...
import { BookOpen, Layers3, Link2, ListPlus, Loader2, Network } from "lucide-react";
import { useNavigate } from "react-router-dom";
import { api, type GraphData, type GraphEdge, type GraphNode } from "@/lib/api";
import { useT } from "@/i18n/I18nProvider";
import { graphSignals, type ClusterSignal, type ConceptSignal, type PaperSignal } from "./graphDecisionSignals";
...
function PaperActionRow({
  ...
  return (
    <div className="rounded-md border border-litera-line px-2.5 py-2">
      ...
      {queue.error && (
        <div className="mt-1 text-[11px] text-litera-error">{(queue.error as Error).message}</div>
      )}
    </div>
  );
}
