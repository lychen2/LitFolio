...
needed to fetch arXiv metadata, parse RSS feeds, and call LLMs. You
...
The idea is that one glance at the list should tell you what these papers are
...
Clicking the status badge cycles through the states in order. The four-state design
reflects the idea that reading a paper once does not mean you are done with it---sometimes
you want to keep important papers close at hand.
...
The input is the paper's \textbf{title + authors + abstract + full PDF body} (truncated
to a 60\,000-character budget if the paper is long). The returned paragraph is saved
directly to the database as a compact summary and appears instantly the next time you
open the paper.

If you have not opened the PDF in the reader yet, LitFolio falls back to a built-in
text extractor and caches the result so subsequent runs are instant.
...
Deep Read now reads the same \textbf{full PDF body} as TL;DR (title + authors +
abstract + full text, truncated to 60\,000 characters). This means the four sections
are grounded in the paper's methodology, experiments, and conclusions rather than just
the abstract---grounded enough to surface details that rarely appear in a typical
abstract, such as specific ablation studies, dataset limitations, or comparison baselines.
...
can have any number of tags. How they differ from folders:
...
immediately reflect the update. Unlike ordinary folders, smart collections do not need
manual drag-in/drag-out; they only care whether the rules match.

\subsection{Performance}

The translated SQL uses SQLite indexes. For tag and folder conditions, the
...
on the Import page. You do not need to visit the Import page
...
term extraction, TL;DR, Deep Read, and full-text indexing read from this cache file
directly; if it is missing, \cmd{lopdf} re-extracts the PDF and writes the cache.
This means AI features do not need to re-parse the PDF once body text is available.
...
\begin{itemize}
  \item \textbf{Already exists}---shows the paper's title, authors, and year. Click
    ``Create link'' to establish a \cmd{builds\_on} citation relationship between the
...
Author surnames are a common source of noise---surnames like ``Chen,''
``Yang,'' and ``Li'' appear frequently in paper author lists, satisfy Title Case
