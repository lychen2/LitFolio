...
返回都没有新条目时，LitFolio 会显示「已穷尽」并停止请求，避免无意义的 API 调用。
...
时会返回 304 Not Modified——这是为什么有时点「刷新」秒回「已是最新」。实际效果是：
...
直接让 LLM 列论文会编造 DOI、作者、年份——幻觉率极高。LitFolio
把「规划」和「检索」分开：LLM 只负责拆子领域和给搜索词，
Semantic Scholar 拉论文由程序代码完成，最后再让 LLM 对真实论文做注释。
...
\begin{enumerate}
  ...
  \item \textbf{多路 FTS5 召回 + 合并评分}——每个搜索词独立跑一次 FTS5 BM25 搜索
    ...
    如果扩写全部返回空，则启动\textbf{逐级降级}检索链：用原始问题做 AND 搜索
    $\rightarrow$ 全部搜索词 OR 模糊匹配 $\rightarrow$ 中文关键词拆分模糊搜索
    $\rightarrow$ 最终回退到最近入库文献，确保 LLM 始终有论文可参考。
  ...
  \item \textbf{LLM 回答}——system prompt 强制要求：只基于提供的证据回答，每个事实
    标注 \cmd{[N]} 引用编号；证据不足时 LLM 会明说,并描述还需要什么类型的论文,而不是
    猜测,用中文回复。回答内容以 Markdown 格式呈现（粗体、斜体、列表、标题等）。
\end{enumerate}

\subsection{@ 精确参考论文}

提问框支持输入 \cmd{@} 主动选择参考论文。输入 \cmd{@} 后会弹出最近入库论文；继续输入
关键词则实时搜索标题/作者/摘要。选中后，论文会以 chip 形式固定在输入框上方，同一问题
可以固定多篇论文。

一旦固定了参考论文，本轮提问会\textbf{跳过自动检索}：后端不会再执行问题改写、FTS5
召回或最近文献兜底，而是只把这些被 \cmd{@} 选中的论文（以及其中的用户高亮）送给 LLM。
这适合「我已经知道要问哪几篇」的场景，能显著提高来源精度，避免自动召回把不相关论文
混进上下文。没有固定论文时，仍走上面的默认 RAG 召回链。
...
\begin{tipbox}
LitFolio 采用\textbf{逐级降级}的检索策略——先用 LLM 改写为精确英文检索词做 AND 匹配，搜不到
就降级为原句搜索、OR 模糊匹配、中文关键词拆分，最后回退到最近入库的文献。这意味着回答
\textbf{始终会拿到一些论文作为参考}。如果召回文献与问题不相关，LLM 会诚实说「这些文献不
直接回答你的问题」而不是强行编造；多轮对话时，后续追问同样基于已召回的文献作答。
\end{tipbox}
