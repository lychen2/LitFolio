...
输入：标题 + 作者（前 6 位）+ 摘要。prompt 要求返回 JSON：\cmd{\detokenize{{"tldr": "一句话，
不超过 40 词", "key_findings": ["3–5 条，每条不超过 20 词"]}}}。输出语言跟随用户
设置。JSON 解析有三级容错（直接解析 → 提取 \cmd{\{...\}} 子串 → 空对象兜底）。
结果写入 \cmd{papers} 表缓存，再次点击不会重调 LLM。

\subsubsection{深读 Quick Read}

输入与 TL;DR 相同（标题 + 作者 + 摘要），但 prompt 更细致：要求返回四段结构化 JSON
（problem / method / comparison / limitations），每段 2–4 句散文体，不能用 bullet。
system prompt 以「资深审稿人替同事判断该不该细读」的角色定位，引导 LLM 给出有判断力
的评价，而非泛泛概括。
...
「整库覆盖」而不是增量合并的设计选择，是因为论文库的修改大多是 PDF 与笔记，
冲突时哪一边胜出的语义对用户难以解释；只暴露「整体推/整体拉」与「自动备份」
反而更可预测。
...
LitFolio 信奉「数据是用户的」。所有信息以可读格式落在 \fpath{\textasciitilde/Litera-Library/}，
你不依赖 LitFolio 一样能用脚本访问。
...
\begin{description}
  ...
  \item[pdf-text.txt] PDF 全文提取（用 pdf-extract 离线生成），用于 FTS5 索引与
    RAG 召回。
\end{description}
...
这不是 bug，而是设计选择：避免每次论文变更都触发规则查询影响性能。
