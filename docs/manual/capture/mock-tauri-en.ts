...
const handlers: Record<string, (a: Args) => unknown> = {
  ...
  paper_translate: () => ({ title: PAPERS[0].title_translated, abstract_text: PAPERS[0].abstract_translated, target_lang: "English", model: "qwen2.5:14b", prompt_tokens: 410, completion_tokens: 280 }),
  draft_translate: () => ({ title: "Attention Is All You Need", abstract_text: "We propose the Transformer, a model architecture relying entirely on attention mechanisms...", target_lang: "English", model: "qwen2.5:14b", prompt_tokens: 230, completion_tokens: 180 }),
  ...
  search_expand_query: () => ({ original: "ultrashort intense laser pulses", expanded: "few-cycle ultrashort laser pulses", terms: ["few-cycle pulse", "post-compression", "attosecond physics"], model: "deepseek-chat", prompt_tokens: 220, completion_tokens: 80 }),
  topic_survey: async () => {
  ...
  note_get: () => "## 速读\n\nThe main contribution of this paper is wrapping metasurface inverse design into an LLM agent.\n\n## 重要数字\n\n- 8.3 rounds average convergence\n- failure recovery reduces attempts by 41%\n- FDTD single-step latency 3 min remains the bottleneck\n\n## 待跟进\n\n- [ ] Verify performance on mesoscale structures\n- [ ] Compare agentic framework cost with traditional topology optimization\n",
  note_save: okVoid,
  reader_translate_selection: () => ({
    translation: "We replace cross-attention in 4 long-document QA backbones with a learnable gating layer, achieving statistically equivalent accuracy on 6/8 benchmarks while saving 23% FLOPs.",
    terms: [
      { term: "cross-attention", local_definition: "An attention sublayer that provides key-value queries between encoder output and decoder.", local_evidence: "see §2 cross-attention sublayer", linked_papers: [{ paper_id: "01PAP4", title: "Attention is all you need", year: 2017, relation: "Originally introduced the cross-attention concept", snippet: "Cross-attention computes queries from the decoder and keys/values from the encoder output." }] },
      { term: "gating layer", local_definition: "A set of learnable gates that dynamically produce weights based on input, determining which dimensions of the feature vector pass through.", local_evidence: "Figure 3 gating block", linked_papers: [] },
    ],
  ...
  // Settings ------------------------------------------------------------
  llm_get_config: () => ({ profiles: PROFILES, active: "DeepSeek", task_assignments: TASK_ASSIGNMENTS, output_language: "English" }),
  ...
  // Feeds ---------------------------------------------------------------
  feeds_list: () => FEEDS,
  feed_add: (a) => ({ id: FEEDS.length + 1, url: String(a?.url), title: "New feed", description: null, etag: null, last_modified: null, last_fetched_at: Math.floor(Date.now() / 1000), last_error: null, created_at: Math.floor(Date.now() / 1000), total_items: 0, unread_items: 0 }),
  feed_remove: okVoid,
  ...
  concept_extract_from_paper: () => [
    { name: "agentic design loop", description: "LLM-driven design-evaluate-correct closed loop", relations: [{ target: "language model", relation: "uses", snippet: "Agent uses LLM for design proposals" }] },
    { name: "FDTD evaluation", description: "Finite-difference time-domain simulation evaluation method", relations: [{ target: "metasurface", relation: "evaluates", snippet: "FDTD evaluates metasurface spectral response" }] },
  ],
