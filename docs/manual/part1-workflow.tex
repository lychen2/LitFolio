...
请求 LLM。换言之，断网你照样能读、能写、能搜索；换台机器只需要把这个目录同步过去。
...
点状态徽章会按顺序循环切换。这套四态机制是 LitFolio 的一个判断：它不假设你看完
一篇文献就万事大吉，而是希望保留「重要的还要常翻」这个语义。

\section{速读 TL;DR}

主列表右侧有「速读」按钮。点一下会调用「TL;DR 生成」任务（默认绑你设置的 \cmd{tldr}
模型）；返回的一段话作为论文摘要的缩写直接写入数据库，下次打开瞬间就有。
...
\begin{itemize}
  ...
  \item 标签 = \textbf{临时主题/项目标记}，朝生暮死，扁平。
\end{itemize}
...
智能收藏夹不是快照——它是每次选中时实时查询的。所以当一篇论文的标签或状态
变更后，下次点击该收藏夹会立即反映变化。这与普通文件夹的语义完全不同：
普通文件夹需要你手动拖入/拖出，智能收藏夹只关心规则是否匹配。
...
在「导入」页粘贴 arXiv ID 后入库。也就是说：你不必专门去「导入」页，刷 RSS 时
看到喜欢的直接入。
...
后端，缓存到 \fpath{papers/<id>/text.txt}。后续的术语提取和 FTS5 索引都直接读这个
缓存文件——只要 \fpath{text.txt} 的修改时间比 \fpath{paper.pdf} 新就复用，否则用
\cmd{lopdf} 重新提取。这意味着：术语生成不需要重新解析 PDF，瞬间可用。
...
\begin{itemize}
  \item \textbf{已存在}——显示该论文的标题、作者和年份，点击「建立引用」即可在当前论文与
    目标论文之间创建一条 \cmd{builds_on} 引用关系（自动去重）。
...
一个容易被忽略的噪声源是作者姓氏——「Chen」「Yang」「Li」这些常见姓氏在论文的作者栏里
