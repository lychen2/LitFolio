...
Input: title + authors (first 6) + abstract + full PDF body text. The body is truncated
to a 60\,000-character budget (head 70\% + tail 30\%) for very long papers. If no cached
body text exists (\fpath{papers/<id>/text.txt}), LitFolio extracts it on the fly via
\cmd{lopdf} and caches the result. The prompt requests JSON: \cmd{\detokenize{{"tldr": "one
sentence, no more than 40 words", "key\_findings": ["3--5 items, each no more than 20
words"]}}}. Output language follows user settings. The system prompt instructs the LLM to
ground every claim in the body text when provided, acknowledging it may be truncated.
JSON parsing has three-level fault tolerance (direct parse $\rightarrow$ extract
\cmd{\{...\}} substring $\rightarrow$ empty object fallback). Results are written to the
\cmd{papers} table cache; clicking again will not re-invoke the LLM.

\subsubsection{Deep Read Quick Read}

Input is the same as TL;DR (title + authors + abstract + full PDF body, truncated to
60\,000 characters). The prompt requests four-section structured JSON (problem / method
/ comparison / limitations), each section 2--4 sentences in prose style, no bullet points
allowed. The system prompt tells the LLM to act like a senior reviewer helping a colleague
decide whether to read the full paper---and to prefer evidence from the body text over
speculation, while acknowledging it may not have seen the entire document. This means
the four sections are now grounded in actual methodology, experiments, and conclusions,
...
Full-library overwrite rather than incremental merging was chosen because
most paper library changes involve PDFs and notes, and the semantics of which
side wins during a conflict are hard to explain clearly. Exposing only ``push all /
...
LitFolio's design principle is that your data belongs to you. All information is stored in
readable formats under \fpath{\textasciitilde/Litera-Library/}, so you can access it with
scripts even without LitFolio.
...
\begin{description}
  ...
  \item[text.txt] Cached full-text extraction from the PDF. Usually written by PDF.js
    when the reader opens the paper; TL;DR / Deep Read may also create it through
    the backend \cmd{lopdf} fallback when no cache exists yet.
\end{description}
...
collection to see the update. This avoids rule queries triggered by every paper change,
